...
//调用
// sort_array_multi($userList, ['total_year', 'parentname'], ['desc', 'asc']);
/**
 * 二位数组排序，支持多字段的升序、降序
 *
 * @param array $arr
...
{
	$curdate  = date("Y-m-d");
	$filepath = SNRUNNING_ROOT . "/logs/" . $curdate;
	if (!is_dir($filepath)) {
	...
	for ($i = 1; $i < $num; $i++) {
		if (is_array($list[$i])) {
			file_put_contents($logfile, date("y-m-d H:i:s") . "#" . json_encode($list[$i]) . PHP_EOL, FILE_APPEND);
		} else {
			file_put_contents($logfile, date("y-m-d H:i:s") . "#" . $list[$i] . PHP_EOL, FILE_APPEND);
		}
	}
	return true;
}

if (!function_exists('PostWeixin')) {
	/**
	 * CURL POST方法
	 * @param [type] $url         url
	 * @param string $data_string json字符串
	 */
