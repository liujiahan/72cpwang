...
/**
 * 是否是可信任的网址来源
 * @author [liu.dongwei] [<email 962823142@qq.com>]
 * @since  [2016-10-08 16:03:30]
 * @param  array  $otherTrustedUrls 额外扩展可信任网址
 * @return boolean                  是否可信任
 */
function trustedReferer($otherTrustedUrls = array())
{
	$urls = array(
		'bocrp.superwx.cn',
	);
	if(is_array($otherTrustedUrls) && $otherTrustedUrls){
		$urls = array_merge($urls, $otherTrustedUrls);
	}

	$trusted = true;
	if(isset($_SERVER['HTTP_REFERER'])){
		$urlinfo = pathinfo($_SERVER['HTTP_REFERER']);
		if(preg_match('/\/\/(.*)/', $urlinfo['dirname'], $m)){
			if(isset($m[1]) && !in_array($m[1], $urls)){
				$trusted = false;
			}
		}
	}
