...
/*
 * 获取部门微信端树Select
 * @param  $pid   int  树的根wxid
 * @param  $selid   int  选中的wxid
 */
function GetDepartTree($pid=1,$selid=0)
{
	global $dosql;
	echo '<select name="wxparentid" id="wxparentid" onchange="seldepart()">';
	echo '<option value="">请选择</option>';
	$parent = $dosql->GetOne("SELECT * FROM `#@__departinfo` WHERE `wxid`='$pid' ");
	$selected = ''; 
	if($parent['wxid']==$selid)
		$selected = 'selected';
	echo '<option value="'.$parent['wxid'].'" '.$selected.'>'.$parent['depart'].'</option>';
	
	$dosql->Execute("select wxid,wxparentid,depart from `#@__departinfo` where wxparentid='$pid' order by wxid asc  ",1);
	while($row1 = $dosql->GetArray(1))
	{
		...
		if($row1['wxid']==$selid)
			$selected = 'selected';
		//$data[] = $row1;
		echo '<option value="'.$row1['wxid'].'" '.$selected.'>'.$row1['depart'].'</option>';
		$dosql->Execute("select wxid,wxparentid,depart from `#@__departinfo` where wxparentid=".$row1['wxid']." order by wxid asc  ",2);
		while($row2 = $dosql->GetArray(2))
		{					
			$selected = ''; 
			if($row2['wxid']==$selid)
				$selected = 'selected';
			echo '<option value="'.$row2['wxid'].'" '.$selected.'>----'.$row2['depart'].'</option>';
			$dosql->Execute("select wxid,wxparentid,depart from `#@__departinfo` where wxparentid=".$row2['wxid']." order by wxid asc  ",3);
			while($row3 = $dosql->GetArray(3))
			{						
				$selected = ''; 
				if($row3['wxid']==$selid)
					$selected = 'selected';
				echo '<option value="'.$row3['wxid'].'" '.$selected.'>--------'.$row3['depart'].'</option>';
			}
		}
	}
	echo '</select>';
...
*/
function GetOrderID($tbname, $pid=0, $pfield='')
{
	global $dosql;

	if($pid>0 && !empty($pfield)){
		$r = $dosql->GetOne("SELECT MAX(orderid) AS `orderid` FROM `$tbname` WHERE `".$pfield."`=$pid");
	}else{
...
function GetContFile($body)
{
	...
	//引入下载类
	require_once(SNRUNNING_DATA.'/httpfile/down.class.php');
 ...
 * @return    void
 */
function PageMsg($title,$msg, $gourl, $onlymsg=0)
{
    $htmlhead  = "<!DOCTYPE html PUBLIC \"-//W3C//DTD XHTML 1.0 Transitional//EN\" \"http://www.w3.org/TR/xhtml1/DTD/xhtml1-transitional.dtd\">\r\n
					...
					<script type=\"text/javascript\" src=\"templates/js/jquery.min.js\"></script></head>\r\n
					<body>";
					
    $htmlhead .= "<div class=\"formHeader\"> <span class=\"title\">提示信息</span></div>";
	
    $htmlfoot  = "\r\n</body>\r\n</html>\r\n";

	$msg .= '&nbsp;&nbsp;&nbsp;&nbsp;[<a href="javascript:history.back(-1);"><font color="#3366CC">返回上页</font></a>]';
    
	$rmsg = "<table width=\"100%\" border=\"0\" cellspacing=\"0\" cellpadding=\"0\" class=\"formTable\">\r\n
			<tr>\r\n
				<td height=\"40\" width=\"10%\" align=\"right\"></td>\r\n
				<td style=\"font-size:14px;color:#CC3333;\"><b>".$title."</b></td>\r\n
			</tr>\r\n
			<tr>\r\n
				<td height=\"40\" align=\"right\"></td>\r\n
				<td style=\"font-size:14px;\">请选择后续的操作：</td>\r\n
			</tr>\r\n
			<tr>\r\n
				<td height=\"40\" align=\"right\"></td>\r\n
				<td style=\"font-size:14px;padding-left:50px;\">".$msg."</td>\r\n
			</tr>\r\n
		</table>
		";
