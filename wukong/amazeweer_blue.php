...
<body>
<div class="topToolbar"> <span class="title">微尔算法蓝号管理</span> 
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="dataTable">
	<tr align="left" class="head">
		<td width="20%" height="36" class="firstCol">ID</td>
		<td width="20%">期号</td>
		<td width="20%">前N位蓝号</td>
		<td width="30%">蓝号及分配概率</td>
		<td width="10%">操作</td>
	</tr>
	<?php

		$sql = "SELECT * FROM `#@__caipiao_weermy_blue` WHERE 1 ";
		...
		while($row = $dosql->GetArray())
		{
			...
			$gailvs   = unserialize($row['gailvs']);
			$total = array_sum($gailvarr);
			$index++;
	?>
	<tr align="left" class="dataTr">
		<td height="36" class="firstCol"><?php echo $index; ?></td>
		<td><?php echo $row['cp_dayid'] ?></td>
		<td><?php echo $row['bluenum'] ?></td>
		<td>
			<?php foreach ($blueinfo as $pos => $blue) {
				echo "蓝号：" . $blue . '分配率' . round($gailvarr[$pos] / $total, 3) * 100 . '%';
				echo "<br/>";
			} ?>
		</td>
		<td>
			<span>[<a href="amazeweer_blue_update.php?id=<?php echo $row['id'] ?>" >修改</a>]</span>
...
?>
<div class="bottomToolbar"> <a href="amazeweer_blue_add.php" class="dataBtn">新增蓝号</a> </div>
...
<div class="quickToolbar">
	<div class="qiuckWarp">
		<div class="quickArea"> <a href="amazeweer_blue_add.php" class="dataBtn">新增蓝号</a> <span class="pageSmall">
